...
==== Custom Tagging
### Custom Tagging
The `observability` config block holds properties related to EG observability extensions (metrics, apm, ...).
From here you can configure custom tag behaviour to decorate the metrics reported to micrometer. Support is available for:
* Route-metadata-tags: supplied items will be used as keys to retrieve values from [route metadata](https://cloud.spring.io/spring-cloud-gateway/reference/html/#route-metadata-configuration). Non-String metadata items will be ignored.
